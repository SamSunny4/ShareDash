...
use crate::discovery::{PairingManager, PeerDiscovery};
use crate::scheduler::metrics::SchedulerTelemetry;
use crate::server::api::{
    cancel_transfer, confirm_pair_session, connect_to_remote_peer, create_pair_session, get_connection_bridges, get_device_info,
    get_discovered_peers, get_pairing_status, get_pending_pair_request, handle_incoming_pair_request,
    list_transfers, open_received_folder, prepare_transfer, respond_to_pair_request, start_benchmark_transfer,
    upload_and_transfer_files, verify_pair_pin, AppState,
};
...
impl Server {
    ...
    pub async fn run(self) -> anyhow::Result<()> {
        ...
        let app = Router::new()
            ...
            .nest_service("/", ServeDir::new(&self.ui_dir))
            .layer(cors)
            .with_state(state);

        let addr = SocketAddr::from(([0, 0, 0, 0], self.port));
        tracing::info!("ShareDash Core Engine HTTP/WS Server running at http://0.0.0.0:{}", self.port);
        println!("🚀 ShareDash Server listening on http://127.0.0.1:{}", self.port);
